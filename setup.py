...
def validate_user_id_string(user_id:str, verbose:bool = True) -> bool:
    ...
    if len(user_id) >= 20 or len(user_id) <= 16:
        if verbose:
            print(f'WARNING: User ID <{user_id}> is invalid lenght (17 - 19 digits)')
        return False
...
def create_admin_file(admin_path:str) -> None:
    with open(admin_path, 'w') as file:
        ...
        while True:
            user_input = input('ADMIN file> ')
            if user_input == 'exit':
                break
            elif validate_user_id_string(user_input):
                continue
            else:
...
if __name__ == '__main__':
    create_dir_structure(f"{os.path.dirname(os.path.abspath(__file__))}")